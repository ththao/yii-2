...
class PaypalComponent extends Component
{
    ...
    {
        if (
            empty($this->API_USERNAME) || empty($this->API_PASSWORD) || empty($this->API_SIGNATURE)
        ) {
            throw new HttpException(403, 'Please configuration Paypal before using.');
        }

        if ($this->MODE == self::MODE_SANDBOX) {// sandbox
    ...
    public function setProducts($products)
    {
        ...
        foreach ($products as $p) {
            ...
            $p_string.="L_NAME" . $i . "=" . $p['NAME'] . "&L_AMT" . $i . "=" . $p['AMOUNT'] . "&L_QTY" . $i . "=" . $p['QTY']."&L_DESC". $i . "=" . $p['DESC'];
            $i++;
        }
    ...
    public function getNVPString()
    {
        ...
        $nvpstr = "&ADDRESSOVERRIDE=1" . $shiptoAddress . "&" . $this->getProductString() . "&ReturnUrl=" . $this->returnURL . "&CANCELURL=" . $this->cancelURL . "&CURRENCYCODE=" . $this->currencyCode. "&SOLUTIONTYPE=Sole";
